...
//对于类型是Number的amf的处理方式，数字直接是1个字节类型+double数据
...
//处理amf是string的，并将amfstring中的data截取出来放到m_obj.amf_string中
//amfstring是一个字节类型+2个字节data长度+数据
int AmfDecoder::decodeString(const char *data, int size, std::string& amf_string)
{
	...
	int bytesUsed = 0;
	int strSize = decodeInt16(data, size);		//将小端数据转化为uint16_t的长度大端
	bytesUsed += 2;
	...
	amf_string = std::string(&data[bytesUsed], 0, strSize);   //存放到成员变量m_obj中
	bytesUsed += strSize;
	return bytesUsed;
}

//处理amf是bool的，一个字节类型+一个字节bool值
...
void AmfEncoder::encodeObjects(AmfObjects& objs)
{
	...
	for (auto iter : objs)
	{
		...
		switch (iter.second.type)    //从AmfObject中可以获取到类型
		{
		case AMF_NUMBER:
